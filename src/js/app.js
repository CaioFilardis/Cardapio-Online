...
cardapio.metodos = {
            ...
            if (validarCep.test(cep)) { // testa se foi validado o cep 
                ...
                $.getJSON("https://viacep.com.br/ws/" + cep + "/json/?callback=?", function (dados) {
                    ...
                    if (!("erro" in dados)) {
                        ...
                        cardapio.metodos.mensagemGlobal('CEP não encontrado. Preencha as informações manualmente');
                        $("#txtEndereco").focus();
                    }
    ...
    finalizarPedido: () => {
        ...
        if (meuCarrinho.length > 0 && meuEndereco != null) {
            ...
            var texto = `Olá, me chamo *${meuEndereco.nome}*, gostaria de fazer um pedido:\n`;
            texto += `\n*Itens do pedido:*\n\${itens}`;
            texto += `\n*Endereço de entrega:*`;
